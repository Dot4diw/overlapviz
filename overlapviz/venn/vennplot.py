...
with open('geometric_data_v3.pkl', 'rb') as f:
...
df_edges = loaded_dict['shape403']['set_edge']
set_label = loaded_dict['shape403']['set_label']
df_label = loaded_dict['shape403']['region_label']

df = pd.read_csv("plotdata_overlaps.csv")
df['set_names']  = df['set_names'].str.replace(' & ', '_')
print(df)
df_label = pd.merge(df_label, df, left_on='id', right_on='set_names', how='left')
print(df_label)
...
for name, group in df_edges.groupby('id'):
    ...
    unique_ids = df_edges['id'].unique()  #  list(df_edges.groupby('id').groups.keys())
    id_to_index = {id_val: i for i, id_val in enumerate(unique_ids)}
    colors = [cm.viridis(i / len(unique_ids)) for i in range(len(unique_ids))]

    index = id_to_index[name]  
    poly = Polygon(verts, 
                closed=True, 
                facecolor=to_rgba(colors[index], 0.4),
                edgecolor='none',
                zorder=1)
...
for _, row in df_label.iterrows():
    ax.text(row['X'], row['Y'], str(row['size']), 
